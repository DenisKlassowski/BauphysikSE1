...
|UC1 |Berechnungen durchführen|Systemnutzer|Berechnung und Ausgabe von j, R~ges~, R~i~, R~T~, U, Δϑ~k~ und ϑ~k~ für in Reihe angeordnete Wandschichten
...
|UC7 |PDF ausgeben |Systemnutzer|Ausgabe auswählbarer Informationen in einem PDF-Dokument
|UC8 |Komplexberechnung durchführen|Systemnutzer| Berechnung und Ausgabe von j, R~ges~, R~i~, R~T~, U, Δϑ~k~ und ϑ~k~ für Bauteile mit Wandschichten in Reihen- und Parallelanordnung
|UC9 |Tauwasserfreiheit berechnen|Systemnutzer|Berechnung der Kondenswasserbildung an Bauteilen|
|===
